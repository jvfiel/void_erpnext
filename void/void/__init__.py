...
def void_so(docname):
    ...
    submit_dn = frappe.db.sql_list("""
    		...
    		where t1.name = t2.parent and t2.against_sales_order = %s and t1.docstatus = 1""", docname)
    ...
    if submit_dn:
        for dn in submit_dn:
            print dn
            frappe.get_doc("Delivery Note", dn).cancel()
            frappe.db.commit()
    ...
    if submit_rv:
        for dn in submit_rv:
            print dn
            si = frappe.get_doc("Sales Invoice", dn)
    ...
    pro_order = frappe.db.sql_list("""
    		...
    		from `tabWork Order`
    		where sales_order = %s and docstatus = 1""", docname)

    if pro_order:
        for dn in submit_rv:
            print dn
            frappe.get_doc("Work Order", dn).cancel()
            frappe.db.commit()
    ...
    frappe.get_doc("Sales Order", docname).cancel()
